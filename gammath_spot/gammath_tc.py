...
def get_closer_lp2(df, lp2_index):
...
def get_closer_lp1(df, lp1_index, lp2_index):
...
def get_closer_hp2(df, hp2_index):
    df_len = len(df)
    j = (df_len-1)
    val = df.Close[j]
    while (j > hp2_index):
...
def get_closer_hp1(df, hp1_index, hp2_index):
    j = hp2_index
    val = df.Close[j]
    while (j > hp1_index):
        if (df.Close[j-1] > val):
...
def draw_correct_support_line(df, lp1_index, lp2_index):
...
def draw_correct_resistance_line(df, hp1_index, hp2_index):
    df_len = len(df)
...
#Plot charts
def draw_trend_charts(tsymbol, path, df, sr_df, y_support_series, y_resistance_series):
...
def compute_support_and_resistance_levels(tsymbol, path, df, need_charts):
    ...
    for i in range(start_index, end_index, step):
        ...
        if (support_line_slope and (lcp > lp)):
            ...
            lp1_index, lp2_index, support_line_slope, support_line_c = draw_correct_support_line(df, lp1_index, lp2_index)
            lp2 = df.Close[lp2_index]
            lp1 = df.Close[lp1_index]
            #project y-value for last x-value
            lp = ((support_line_slope*(df_len-1)) + support_line_c)
        ...
        if (lcp > lp): #Valid line found
            ...
            if (lcp > lp2): #Check if we should use lp2 instead
                if (diff_lp2 <= diff_2p):
                    ...
                    support_line_slope = 0
                    #Find a closer lp2
                    lp2_index = get_closer_lp2(df, lp2_index)
                    start_point = lp2_index
            ...
            #Find a closer lp1
            lp1_index = get_closer_lp1(df, lp1_index, lp2_index)
    ...
    for i in range(start_index, end_index, step):
        ...
        if (resistance_line_slope and (lcp < lp)):
            #Relevant line. Check if interim points are all less than line points
            hp1_index, hp2_index, resistance_line_slope, resistance_line_c = draw_correct_resistance_line(df, hp1_index, hp2_index)
            hp2 = df.Close[hp2_index]
            hp1 = df.Close[hp1_index]
            #project y-value for last x-value
            lp = ((resistance_line_slope*(df_len-1)) + resistance_line_c)
        ...
        if (lcp < lp): #Valid line found
            ...
            if (lcp < hp2):
                if (diff_hp2 <= diff_2p): #Check if we should use hp2 instead
                    ...
                    resistance_line_slope = 0
                    #Find a closer hp2
                    hp2_index = get_closer_hp2(df, hp2_index)
                    start_point = hp2_index
            ...
            if (hp1 > lcp):
                #Find a closer hp1
                hp1_index = get_closer_hp1(df, hp1_index, hp2_index)
