...
1. Price projection chart and projections are saved in `tickers/*symbol*` dir. `*symbol*_pep.png` shows the chart and `*symbol*_pp.csv` shows the projected values. A sorted list of moving estimated projected 5Y returns are saved in `tickers/MPEP.csv`
